...
By default, KubeDB expects the oplog of the secondary to sync with the primary, and the objects count of a database in a secondary to match with the primary while restarting the database. If you are performing ops requests in an active database, the oplog may not sync, or the objects count may not match while restarting. Then you can specify the `spec.readinessCriteria` section of the ops request. This field consists of the following sub-field:

- `spec.readinessCriteria.oplogMaxLagSeconds` specifies the maximum oplog in seconds that a secondary can lag from the primary while restarting.
- `spec.readinessCriteria.objectsCountDiffPercentage` specifies the allowed percentage of objects count difference of a database from master to secondary while restarting.

### spec.timeoutSeconds

By default, each ops request has a timeout of 600 seconds (10 minutes) for each step in the ops request, that means the ops request will fail if any of the steps in the ops request takes more than 600 seconds. If you want to modify this timeout, you can set the `spec.timeoutSeconds` to your desired value in seconds.
...
| Phase      | Meaning                                                                            |
| ---------- | ---------------------------------------------------------------------------------- |
| Pending    | KubeDB has not started performing the operation requested in the MongoDBOpsRequest |
| Progressing| KubeDB is currently performing the operation requested in the MongoDBOpsRequest    |
